...
const NavItem = ({ href, label, icon, active, onClick }) => (
  <button
    type="button"
...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { href: "/admin/health", label: "Health", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
  ];
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 min-h-screen bg-surface-900/50 border-r border-surface-700/30 p-5 flex flex-col shrink-0 transform transition-transform duration-200 ${
        ...
        <div className="pt-4 border-t border-surface-700/30 space-y-1">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-surface-400 hover:text-red-400 hover:bg-red-500/10 transition border border-transparent"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
        ...
        <header className="sticky top-0 z-30 bg-navy-800/80 backdrop-blur-xl border-b border-surface-700/30 px-4 lg:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="lg:hidden w-8 h-8 rounded-lg flex items-center justify-center text-surface-400 hover:text-white hover:bg-surface-800 transition"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
