...
import { Users, Globe, MessageCircle, Clock, XCircle, CircleAlert } from "lucide-react";
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4" aria-live="polite">
        ...
        <button type="button" aria-label="Retry loading admin data" onClick={() => window.location.reload()} className="px-5 py-2 rounded-xl bg-brand-500/15 text-brand-300 text-sm font-medium hover:bg-brand-500/25 transition border border-brand-500/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
          Retry
        </button>
      </div>
    );
