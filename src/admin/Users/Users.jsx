    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          ...
          <button key={item.tab} type="button" aria-label={item.label} onClick={() => { setSummaryTab(item.tab); setPage(1); }}
            className={`relative overflow-hidden rounded-2xl p-5 bg-gradient-to-br ${item.gradient} shadow-lg animate-fade-in transition-all hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${summaryTab === item.tab ? "ring-2 ring-white/20" : ""}`}>
        ...
        <div className="p-5 border-b border-surface-700/30 space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {FILTERS.map((f) => (
                <button key={f.value} type="button" aria-label={"Filter: " + f.label} onClick={() => setFilter(f.value)}
                  className={`px-3 py-2 rounded-xl text-xs font-medium transition border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${
                    filter === f.value
                ...
                displayUsers.map((u) => (
                  <tr key={u._id} className="hover:bg-surface-800/30 transition">
                    <td className="px-5 py-4">
                      <button type="button" aria-label={"View " + u.name + " details"} onClick={() => viewUserDetail(u._id)} className="flex items-center gap-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 justify-end flex-wrap">
                        <button type="button" aria-label={"View " + u.name + " details"} onClick={() => viewUserDetail(u._id)} className="px-2.5 py-1.5 rounded-xl text-[11px] font-medium bg-brand-500/10 text-brand-400 border border-brand-500/20 hover:bg-brand-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">View</button>
                        <button type="button" aria-label={(u.isSuspended ? "Unsuspend " : "Suspend ") + u.name} onClick={() => setConfirmAction({ type: "suspend", userId: u._id })}
                          className={`px-2.5 py-1.5 rounded-xl text-[11px] font-medium transition border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${u.isSuspended ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/20"}`}>
                          {u.isSuspended ? "Unsuspend" : "Suspend"}
                        </button>
                        {u.role !== "admin" && (
                          <button type="button" aria-label={"Delete " + u.name} onClick={() => setConfirmAction({ type: "delete", userId: u._id })}
                            className="px-2.5 py-1.5 rounded-xl text-[11px] font-medium bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">Delete</button>
                        )}
                      </div>
                    </td>
                  </tr>
        ...
        {!isOnlineTab && totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-surface-700/30">
            ...
            <div className="flex gap-2">
              <button type="button" aria-label="Previous page" onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1}
                className="px-4 py-2 rounded-xl text-xs font-medium bg-surface-800/50 text-surface-300 border border-surface-700/30 hover:bg-surface-700/50 transition disabled:opacity-40 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">Previous</button>
              <button type="button" aria-label="Next page" onClick={() => setPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages}
                className="px-4 py-2 rounded-xl text-xs font-medium bg-surface-800/50 text-surface-300 border border-surface-700/30 hover:bg-surface-700/50 transition disabled:opacity-40 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">Next</button>
            </div>
          </div>
        )}
      </div>
            ...
            {detailLoading ? (
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button type="button" aria-label="Close user details" onClick={() => { setSelectedUser(null); setUserDetail(null); }} className="text-surface-500 hover:text-white transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 border-t border-surface-700/30 pt-4">
                  <button type="button" aria-label={(userDetail.isSuspended ? "Unsuspend " : "Suspend ") + userDetail.name} onClick={() => { setConfirmAction({ type: "suspend", userId: userDetail._id }); }}
                    className={`px-4 py-2 rounded-xl text-xs font-medium transition border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${userDetail.isSuspended ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/20"}`}>
                    {userDetail.isSuspended ? "Unsuspend" : "Suspend"}
                  </button>
                  <button type="button" aria-label={"Change role of " + userDetail.name} onClick={() => { setRoleModal(userDetail._id); setNewRole(userDetail.role === "admin" ? "user" : "admin"); }}
                    className="px-4 py-2 rounded-xl text-xs font-medium bg-brand-500/10 text-brand-400 border border-brand-500/20 hover:bg-brand-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                    Change Role
                  </button>
                  <button type="button" aria-label={"Toggle verify for " + userDetail.name} onClick={() => { setConfirmAction({ type: "verify", userId: userDetail._id }); }}
                    className="px-4 py-2 rounded-xl text-xs font-medium bg-teal-500/10 text-teal-400 border border-teal-500/20 hover:bg-teal-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                    Toggle Verify
                  </button>
                  <button type="button" aria-label={"Reset password for " + userDetail.name} onClick={() => { setPasswordModal(userDetail._id); setNewPassword(""); }}
                    className="px-4 py-2 rounded-xl text-xs font-medium bg-violet-500/10 text-violet-400 border border-violet-500/20 hover:bg-violet-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                    Reset Password
                  </button>
                  {userDetail.role !== "admin" && (
                    <button type="button" aria-label={"Delete " + userDetail.name} onClick={() => { setConfirmAction({ type: "delete", userId: userDetail._id }); }}
                      className="px-4 py-2 rounded-xl text-xs font-medium bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
                      Delete
                    </button>
                  )}
                </div>
      ...
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4 ${roleModal ? "" : "hidden"}`}>
        <div className="bg-surface-800 border border-surface-700/50 rounded-2xl p-6 w-full max-w-sm shadow-glass animate-scale-in">
          ...
          <div className="flex gap-3 mb-6">
            <button type="button" aria-label="Set role to user" onClick={() => setNewRole("user")}
              className={`flex-1 px-4 py-3 rounded-xl text-sm font-medium transition border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${newRole === "user" ? "bg-brand-500/15 text-brand-300 border-brand-500/20" : "bg-surface-800/50 text-surface-400 border-surface-700/30 hover:text-white"}`}>User</button>
            <button type="button" aria-label="Set role to admin" onClick={() => setNewRole("admin")}
              className={`flex-1 px-4 py-3 rounded-xl text-sm font-medium transition border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${newRole === "admin" ? "bg-brand-500/15 text-brand-300 border-brand-500/20" : "bg-surface-800/50 text-surface-400 border-surface-700/30 hover:text-white"}`}>Admin</button>
          </div>
          <div className="flex gap-3 justify-end">
            <button type="button" aria-label="Cancel role change" onClick={() => setRoleModal(null)} disabled={actionLoading}
              className="px-5 py-2.5 rounded-xl text-sm font-medium text-surface-300 hover:text-white hover:bg-surface-700 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">Cancel</button>
            <button type="button" aria-label="Confirm role change" onClick={handleRoleChange} disabled={actionLoading}
              className="px-5 py-2.5 rounded-xl text-sm font-medium bg-brand-500/15 text-brand-300 border border-brand-500/20 hover:bg-brand-500/25 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
      ...
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4 ${passwordModal ? "" : "hidden"}`}>
        <div className="bg-surface-800 border border-surface-700/50 rounded-2xl p-6 w-full max-w-sm shadow-glass animate-scale-in">
          ...
          <div className="flex gap-3 justify-end">
            <button type="button" aria-label="Cancel password reset" onClick={() => { setPasswordModal(null); setNewPassword(""); }} disabled={actionLoading}
              className="px-5 py-2.5 rounded-xl text-sm font-medium text-surface-300 hover:text-white hover:bg-surface-700 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">Cancel</button>
            <button type="button" aria-label="Confirm password reset" onClick={handlePasswordReset} disabled={actionLoading || newPassword.length < 8}
              className="px-5 py-2.5 rounded-xl text-sm font-medium bg-brand-500/15 text-brand-300 border border-brand-500/20 hover:bg-brand-500/25 transition disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
