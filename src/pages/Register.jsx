        ...
        <form onSubmit={handleRegister}>
          ...
          {formData.password && (
            <div className="mb-4 -mt-3">
              <div className="flex gap-1.5 mb-1" role="meter" aria-valuenow={passwordStrength.score} aria-valuemin={0} aria-valuemax={6} aria-label="Password strength">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                      i < passwordStrength.score
                        ? strengthColors[Math.min(passwordStrength.score - 1, 5)] || "bg-emerald-500"
                        : "bg-surface-700"
                    }`}
                  />
                ))}
              ...
              <div className="mt-2 grid grid-cols-2 gap-1">
                {[
                  ...
                  <div key={req.key} className="flex items-center gap-1.5 text-[10px]" aria-label={`${req.label} ${passwordStrength.requirements[req.key] ? "met" : "not met"}`}>
