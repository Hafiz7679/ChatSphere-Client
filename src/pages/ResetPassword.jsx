        ...
        <form onSubmit={handleSubmit}>
          ...
          {password && (
            <div className="mb-4 -mt-3">
              <div className="flex gap-1.5 mb-1" role="meter" aria-valuenow={strength.score} aria-valuemin={0} aria-valuemax={6} aria-label="Password strength">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                      i < strength.score
                        ? ["bg-red-500", "bg-orange-500", "bg-amber-500", "bg-lime-500", "bg-emerald-500", "bg-emerald-500"][Math.min(strength.score - 1, 5)]
                        : "bg-surface-700"
                    }`}
                  />
                ))}
