...
import { Clock, Check, CheckCheck, Pause, Play, Download, EllipsisVertical } from "lucide-react";
...
const StatusIcon = ({ status }) => {
  ...
  return status === "sent" ? (
    <Check className={`w-3 h-3 ${status === "read" ? "text-accent-400" : status === "delivered" ? "text-emerald-400" : "text-surface-400"}`} />
  ) : (
    <CheckCheck className={`w-3 h-3 ${status === "read" ? "text-accent-400" : status === "delivered" ? "text-emerald-400" : "text-surface-400"}`} />
  );
};
                ...
                <div className={`relative flex items-end gap-2 mb-1 ${isOwn ? "justify-end" : "justify-start"}`}>
                  <div className="group relative max-w-[80%] md:max-w-[65%]">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-0 right-0 -mt-1 -mr-1">
                      ...
                      >
                        <EllipsisVertical className="w-3.5 h-3.5" fill="currentColor" />
                      </button>
                    </div>
