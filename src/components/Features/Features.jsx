import { MessageCircle, Lock, Zap, Users, Cloud, Smartphone } from "lucide-react";

const features = [
  { icon: <MessageCircle />, title: "Real-Time Messaging", description: "Instant conversations powered by Socket.IO with seamless message delivery." },
  { icon: <Lock />, title: "Secure Authentication", description: "JWT authentication and encrypted communication keep your account protected." },
  { icon: <Zap />, title: "Lightning Fast", description: "Optimized React frontend and Node.js backend for blazing-fast performance." },
  { icon: <Users />, title: "Online Presence", description: "Know who's online instantly and enjoy a smooth real-time chat experience." },
  { icon: <Cloud />, title: "Cloud Ready", description: "Designed to deploy on Vercel, Render, Railway or any cloud platform." },
  { icon: <Smartphone />, title: "Responsive Design", description: "Works beautifully on desktop, tablet and mobile devices." },
];
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((feature, index) => (
            <div key={index}
              className={`group relative rounded-3xl border p-6 md:p-8 transition-all duration-300 hover:-translate-y-2 hover:border-brand-500/50 hover:shadow-glow ${index === 0 ? "border-brand-500/20 bg-brand-600/5 backdrop-blur-xl" : "border-surface-700/30 bg-surface-800/30 backdrop-blur-xl"}`}>
              ...
              <div className="mt-6 md:mt-8 h-1 w-14 rounded-full bg-gradient-to-r from-brand-500 to-accent-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </div>
          ))}
        </div>
      </div>
