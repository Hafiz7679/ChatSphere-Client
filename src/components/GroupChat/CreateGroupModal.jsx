...
const CreateGroupModal = ({ users, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Create group" onClick={onClose}>
      <div className="bg-navy-900 border border-surface-700/30 rounded-2xl shadow-glass w-full max-w-md max-h-[80vh] flex flex-col animate-scale-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-surface-700/30">
          ...
          <button type="button" aria-label="Close create group modal" onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-surface-400 hover:text-white hover:bg-surface-800 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-5 border-b border-surface-700/30">
          <input type="text" placeholder="Group name" value={name} onChange={(e) => setName(e.target.value)}
            ref={nameRef} aria-label="Group name"
          ...
          {users.filter((u) => u._id).map((user) => {
            ...
            return (
              <button key={user._id} type="button" onClick={() => toggle(user._id)}
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50 ${sel ? "bg-brand-500/10" : "hover:bg-surface-800/50"}`}>
        ...
        <div className="p-5 border-t border-surface-700/30">
          <Button onClick={handleCreate} disabled={loading || !name.trim() || selected.length < 2} className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/50">
